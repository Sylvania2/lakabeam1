	...
	void scan_publish()
	{
		...
		while (rclcpp::ok())
		{
				...
				while(1)
				{
					...
					for(;j < 12; j++)
					{
						for(i = 0; i < 16; i++)
						{
							...
							response_ptr.angle = (MSOP_Data.BlockID[j].Azimuth + (resolution * 2 * i));
			...
			if(scan_vec_ready == 1)
			{
				...
				scan.angle_increment = 2.0 * M_PI / (num_readings/2);
				scan.scan_time = duration;
				scan.time_increment = duration/(float)(num_readings/2);
				scan.range_min = 0.0;
				scan.range_max = 100.0;
				scan.ranges.resize(num_readings/2);
				scan.intensities.resize(num_readings/2);

				for(int i = 0;i < num_readings; i++)
				{
					if (!inverted)
					{
						if((i>(num_readings/8)) & (i< (num_readings*7/8) ) & (i%2==1))
						{
							scan.ranges[i/2] = (float)scan_vec[i].dist / 1000;
							scan.intensities[i/2] = scan_vec[i].rssi;
					...
					else
					{
						...
						scan.ranges[(num_readings - i - 1)/2] = (float)scan_vec[i].dist / 1000;
						scan.intensities[(num_readings - i - 1)/2] = scan_vec[i].rssi;
				    	if(scan.ranges[(num_readings - i - 1)/2] == 0)
						{
							scan.ranges[(num_readings- i - 1)/2] = inf;
							scan.intensities[(num_readings - i - 1)/2] = 0;
						}
						}
						else
						{
